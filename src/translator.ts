...
SELECT COALESCE(json_group_array(CAST(n AS INTEGER)), json_array()) FROM r)`, 
   ...
   */
  private translateExistsPattern(expr: Expression): { sql: string; tables: string[]; params: unknown[] } {
    const tables: string[] = [];
    const params: unknown[] = [];
   ...
   */
  private translateSizePattern(expr: Expression): { sql: string; tables: string[]; params: unknown[] } {
    const tables: string[] = [];
    const params: unknown[] = [];
